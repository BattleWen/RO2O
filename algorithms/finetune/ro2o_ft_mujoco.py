...
class RO2O:
    ...
    def _actor_loss(self, state: torch.Tensor, action: torch.tensor) -> Tuple[torch.Tensor]:
        ...
        batch_entropy = - pi_log_prob.mean()
    ...
    def _critic_loss(
        ...
        with torch.no_grad():
            ...
            q_next_min, _ = q_next.min(dim=0)
            q_next_min_expanded = q_next_min.unsqueeze(0).expand_as(q_next)            
            q_target = reward.unsqueeze(-1) + self.gamma * (1 - done.unsqueeze(-1)) * q_next_min_expanded.unsqueeze(-1)
    ...
    def update_online(self, batch: TensorBatch) -> Dict[str, float]:
        ...
        # Actor update
        actor_loss, q_loss, policy_loss, batch_entropy, q_value_std = self._actor_loss(state, action)
...
def online_ft(config:TrainConfig):
    ...
    # csv_file_path = 'RO2O_buffer_data2.csv'
    # with open(csv_file_path, mode='w', newline='') as file:
    #     writer = csv.writer(file)
    #     writer.writerow(['State', 'Action', 'Reward', 'Next State', 'Done'])  # 写入CSV文件的列名
