...
class RO2O:
    ...
    def _actor_loss(self, state: torch.Tensor, action: torch.tensor) -> Tuple[torch.Tensor]:
        ...
        return loss, q_loss, bc_loss, policy_loss, batch_entropy, q_value_std
    ...
    def update(self, batch: TensorBatch) -> Dict[str, float]:
        ...
        # Actor update
        actor_loss, q_loss, bc_loss, policy_loss, batch_entropy, q_values_std = self._actor_loss(state, action)
    ...
    for epoch in trange(config.num_epochs, desc="Training"):
        ...
        if epoch % config.eval_every == 0 or epoch == config.num_epochs - 1:
            eval_returns = eval_actor(
                ...
                seed=config.eval_seed + config.train_seed,
                device=config.device,
            )
            eval_log = {
...
if __name__ == "__main__":
    ...
    parser.add_argument('--group', default="RORL", type=str)
    parser.add_argument('--name', default="RORL", type=str)
    ...
    parser.add_argument('--env_name', default="antmaze-large-play-v2", type=str)
    ...
    parser.add_argument('--beta_bc', default=0.0, type=float)
